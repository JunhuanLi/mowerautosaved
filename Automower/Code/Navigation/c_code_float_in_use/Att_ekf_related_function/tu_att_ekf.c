 ...
 * Return Type  : void
 */
void tu_att_ekf(float P[16], float stateVec[4], float
                angular_imu_rad_s[3], float dt, float Q[9], float
                P_updated[16], float stateVec_updated[4])
{
  signed char I[16];
  int k;
  float F_discrete[16];
  ...
  for (k = 0; k < 4; k++) {
    for (i0 = 0; i0 < 4; i0++) {
      b_F_discrete[i0 + (k << 2)] = 0.5 * F_discrete[i0 + (k << 2)] + (float)
        I[i0 + (k << 2)];
    }
  }
