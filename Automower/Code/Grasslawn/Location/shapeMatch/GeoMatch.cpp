...
int GeoMatch::CreateGeoMatchModel(const void *templateArr,double maxContrast,double minContrast)
{
	...
	for(int m=0;m<noOfCordinates ;m++)
	{
		...
		if(minModelRow > cordinates[m].x)
			minModelRow = cordinates[m].x ;
		if(maxModelRow < cordinates[m].x)
			maxModelRow = cordinates[m].x ;

		if(minModelCol > cordinates[m].y)
			minModelCol = cordinates[m].y ;
		if(maxModelCol < cordinates[m].y)
			maxModelCol = cordinates[m].y ;
	}

	if(minModelCol < 0)
		minModelCol = -minModelCol;
	if(minModelRow < 0)
		minModelRow = -minModelRow;

	cout<<minModelCol<<"  "<<maxModelCol<<"  "<<minModelRow<<"  "<<maxModelRow<<endl;
...
double GeoMatch::FindGeoMatchModel(const void* srcarr,double minScore,double greediness,CvPoint *resultPoint)
{
	...
	CvMat *matGradMag = cvCreateMat( Ssize.height, Ssize.width, CV_32FC1 );
	...
	for( i = 0; i < Ssize.height; i++ )
    {
		 ...
		 float *_matGradMag = (float*)(matGradMag->data.ptr + matGradMag->step * i);
		 for( j = 0; j < Ssize.width; j++ )
		{
				...
				if(gradMag>0) // hande divide by zero
					_matGradMag[j]=1/gradMag;   // 1/Sqrt(dx^2 +dy^2)
				else
					_matGradMag[j]=0;
	...
	for( i = minModelRow; i < Ssize.height-maxModelRow; i++ )
    {
		short *_dx = (short*)(Sdx->data.ptr + Sdx->step*i);
		short *_dy = (short*)(Sdy->data.ptr + Sdy->step*i);
		float *_GradMag = (float*)(matGradMag->data.ptr + matGradMag->step * i);
	    for( j = minModelCol; j < Ssize.width - maxModelCol; j++ )
        { 
			partialSum = 0; // initilize partialSum measure
			for(m=0;m<noOfCordinates ;m ++)
		    {
				int _curX = cordinates[m].x;
	            int _curY = cordinates[m].y;
				curX = i + _curX ;	// template X coordinate
				curY = j + _curY ; // template Y coordinate

				iTx	= edgeDerivativeX[m];	// template X derivative
				iTy	= edgeDerivativeY[m];    // template Y derivative
				
				int deviate =  width * _curX;
				_Sdx = (short*)(_dx + deviate);
				_Sdy = (short*)(_dy + deviate);
				...
				float *_matGradMag = (float*)(_GradMag + deviate);
						
				partialSum = partialSum + ( (iSx * iTx) + (iSy * iTy) ) * (edgeMagnitude[m] * _matGradMag[curY]);
