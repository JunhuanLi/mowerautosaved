function [K_norm,P_updated,delta_stateVec_updated,stateVec_updated] = gps_pos_mu_18(P,delta_stateVec,pos_gps_lla,stateVec,R,pos_lla_ref)
%#codegen
...
gps_pos_ecef = wgslla2xyz(pos_gps_lla(1),pos_gps_lla(2),100);
gps_pos_ned = wgsxyz2ned(gps_pos_ecef,pos_lla_ref(1),pos_lla_ref(2),100);
measurement_gps = gps_pos_ned;

%method 1
% H = [eye(6) zeros(6,13)];
% K = P * H'/(H * P * H'+R);
% Z = x(1:6) - measurement_gps;
% delta_x_updated = K * (Z - H * delta_x);
% P_updated = (eye(length(P)) - K * H) * P;
...
stateVec_delta_stateVec = zeros(19,18);
...
%full state update
stateVec_updated = zeros(19,1);
stateVec_updated(1:6) = stateVec(1:6) + delta_stateVec_updated(1:6);
stateVec_updated(11:19) = stateVec(11:19) + delta_stateVec_updated(10:18);
stateVec_updated(7:10) = cross_quat(eul2quat(delta_stateVec_updated(7:9)'),stateVec(7:10)');
stateVec_updated(7:10) = stateVec_updated(7:10)/norm(stateVec_updated(7:10));

%% EKF reset
G = eye(18);
G(7:9,7:9) = eye(3) - cross3(0.5*delta_stateVec(7:9));   
