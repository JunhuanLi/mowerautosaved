 ...
 * Return Type  : void
 */
void mu_att_ekf(double P[16], double acc_imu_g[3], double stateVec[4],
                double R[9], double P_updated[16], double stateVec_updated[4])
{
  ...
  for (rtemp = 0; rtemp < 4; rtemp++) {
    for (k = 0; k < 4; k++) {
      ...
      b_I[rtemp + (k << 2)] = (double)I[rtemp + (k << 2)] - maxval;
    }
