...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    objs = []
    # lib
    objs.extend(SConscript(Rtt_Root + '/StmLib/SConscript', variant_dir=bsp_vdir + '/lib', duplicate=0))
    # board build script
    objs.extend(SConscript(Rtt_Root + '/src/SConscript', variant_dir=bsp_vdir + '/src', duplicate=0))
    # include application
    objs.extend(SConscript(Rtt_Root + '/Application/SConscript', variant_dir=bsp_vdir + '/app', duplicate=0))
    # include Algorithm
    objs.extend(SConscript(Rtt_Root + '/Algorithm/SConscript', variant_dir=bsp_vdir + '/algo', duplicate=0))
    # include Communication
    objs.extend(SConscript(Rtt_Root + '/Communication/SConscript', variant_dir=bsp_vdir + '/com', duplicate=0))
    # include driver
    objs.extend(SConscript(Rtt_Root + '/Driver/SConscript', variant_dir=bsp_vdir + '/driver', duplicate=0))
    
    # include kernel
    objs.extend(SConscript(Rtt_Root + '/OS/src/SConscript', variant_dir=kernel_vdir + '/src', duplicate=0))
    # include libcpu
    if not has_libcpu:
        objs.extend(SConscript(Rtt_Root + '/OS/libcpu/SConscript',
                    variant_dir=kernel_vdir + '/libcpu', duplicate=0))

    # include components
    objs.extend(SConscript(Rtt_Root + '/OS/components/SConscript',
                           variant_dir=kernel_vdir + '/components',
                           duplicate=0,
                           exports='remove_components'))
...
def EndBuilding(target, program = None):
    ...
    #do_copy_file(rtconfig.TARGET, os.getcwd() + '/build')
