...
{										    
	...
	s32 latitude;				//纬度 分扩大10000000倍,实际要除以10000000
	u8 nshemi;					//北纬/南纬,N:北纬;S:南纬				  
	s32 longitude;			//经度 分扩大10000000倍,实际要除以10000000
  ...
  s32 height;         //地球表面高度 mm
	s32 hMSL;           //海平面高度 mm
	u8 fixType;         //定位类型，0x00 = No Fix 0x01 = Dead Reckoning only 0x02 = 2D-Fix 0x03 = 3D-Fix 0x04 = GNSS + dead reckoning combined 0x05 = Time only fix
	u32 hAcc;           //水平精度估计 mm
	u32 vAcc;           //垂直精度估计 mm
	u32 sAcc;           //速度精度估计 mm/s
	s32 velN;           //north velocity mm/s
	s32 velE;           //north velocity mm/s
	s32 velD;           //north velocity mm/s
	u16 pdop;           //单位：0.01
	s32 headMot;        //heading of motion, deg
	int altitude;			 	//海拔高度,放大了1000倍,实际除以1000.单位:mm	 
